var BoxCritters = (function() {
	 ...
	 * "ticker" - Dispatched by the ticker.
	 * "update" - Dispatched every 2.5 seconds.
	 * "playerJoin", {joinCrumb} - Dispatched when a player join to the room.
	 * "playerLeave", {leaveCrumb} - Dispatched when a player leaves the room.
	 * "playerMove", {moveCrumb} - Dispatched when a player moves.
	 * "playerMessage", {messageCrumb} - Dispatched when a player sends a message.
	 * 
	 */
	var eventHandler = new BCEventHandler();
	...
	function Room(t, o, e) {
		if (
			...
			this.game.addEventListener("tick", function(t) {
				eventHandler.dispatchEvent("ticker", t);
			}),
			e.artwork &&
				e.artwork.sprites &&
				(console.log(e.artwork.sprites.images[0]),
			...
			var room_bg_string = e.artwork.background.replace(/(.*)\/(.*)\.png/, "$2");
			console.log("Add Background:", room_bg_string);
			var r = new createjs.Bitmap(this.artwork.roomPath + e.artwork.background/*/queue.getResult(room_bg_string)*/);
		...
		if (
			...
			var room_fg_string = e.artwork.foreground.replace(/(.*)\/(.*)\.png/, "$2");
			console.log("Add Foreground:", room_fg_string);
	...
	function World(r, t, o) {
		...
		this.stage.on("stagemousedown", function(t) {
			var tmp_xy = t.target.globalToLocal(t.stageX, t.stageY);
			var xy = {
				x: Math.floor(tmp_xy.x),
				y: Math.floor(tmp_xy.y)
			};
			eventHandler.dispatchEvent("click", xy);
			e.socket.emit("click", xy);
		}),
	...
	function Coins(s, q)
	{
		...
		var coins_bg = new createjs.Bitmap("{{url}}/media/ui/coins.png");
	...
	(Room.prototype.sortDepth = function() {
		...
		(World.prototype.eventHandler = function(o) {
			//Setup Events
			o.addEventListener("update", this.updateEvent);
		}),
